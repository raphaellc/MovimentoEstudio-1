...
public: 
	Lista();
	~Lista() {};
	void inserirLista(T elemento); //Insere um Nó com dado no próximo espaço vago
	void removerLista(); //Remove o último elemento da lista
	T buscarLista(int pos); //Retorna o dado de um Nó na posição especificada
	int quantidadeElementos(); //Retorna quantos nos tem na lista
	T obterPrimeiroElementoObsoleto(); //Retorna o dado do primeiro no da lista
	T obterUltimoElementoObsoleto(); //Retorna o dado do ultimo elemento
